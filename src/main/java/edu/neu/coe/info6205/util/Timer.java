...
public class Timer {
     ...
     * @return the average milliseconds per repetition.
     */
    public <T,U>  double repeat(int n, Supplier<T> supplier, Function<T,
            U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        logger.trace("repeat: with " + n + " runs");
        if(running) {
            pause();
        }
        for(int i=0;i<n;i++) {
            T t_type = supplier.get();
            if (preFunction!=null) {
                t_type = preFunction.apply(t_type);
            }

            resume();

            U u = function.apply(t_type);
            pauseAndLap();
    ...
    private static long getClock() {
        ...
        Instant instnt = clk.instant();
        return instnt.getNano();
    }
